...
export default function Header() {
    ...
    const isActive = (href) => {
        return currentPath === href ? 'bg-gray-700 text-white' : 'hover:bg-gray-700 hover:text-white';
    };
    ...
    return (
        <div className="text-white mt-[-10px] flex justify-between items-center px-4 py-3 sm:px-6 md:px-8 lg:px-12">
            ...
            <div className="hidden md:flex space-x-4 mt-5">
                <a href="/" className={`rounded-md px-3 py-2 text-sm font-medium text-gray-300 ${isActive('/')}`} aria-current="page">Эхлэл</a>
                <a href="/comingsoon" className={`rounded-md px-3 py-2 text-sm font-medium text-gray-300 ${isActive('/comingsoon')}`}>Тун удахгүй</a>
                <a href="/hutulbur" className={`rounded-md px-3 py-2 text-sm font-medium text-gray-300 ${isActive('/hutulbur')}`}>Цагийн хуваарь</a>
                <a href="/uilchilgee" className={`rounded-md px-3 py-2 text-sm font-medium text-gray-300 ${isActive('/uilchilgee')}`}>Үйлчилгээ</a>
                <a href="/bonus" className={`rounded-md px-3 py-2 text-sm font-medium text-gray-300 ${isActive('/bonus')}`}>Бонус карт</a>
            </div>
            ...
            {menuOpen && (
                <div className="md:hidden fixed top-0 left-0 right-0 bg-gray-900 text-white p-4 mt-12 flex flex-col space-y-4 z-50">
                    ...
                    <a 
                        href="/comingsoon" 
                        className={`text-lg font-semibold transition-all duration-300 ${isActive('/comingsoon')}`} 
                        onClick={() => setMenuOpen(false)}>
                        Тун удахгүй
                    </a>
                    <a 
                        href="/hutulbur" 
                        className={`text-lg font-semibold transition-all duration-300 ${isActive('/hutulbur')}`} 
                        onClick={() => setMenuOpen(false)}>
                        Цагийн хуваарь
                    </a>
                    <a 
                        href="/uilchilgee" 
                        className={`text-lg font-semibold transition-all duration-300 ${isActive('/uilchilgee')}`} 
                        onClick={() => setMenuOpen(false)}>
                        Үйлчилгээ
                    </a>
                    <a 
                        href="/bonus" 
                        className={`text-lg font-semibold transition-all duration-300 ${isActive('/bonus')}`} 
                        onClick={() => setMenuOpen(false)}>
                        Бонус карт
                    </a>
                    <a 
