...
// Define the Coming type structure
interface Coming {
  _id: string;
  title: string;
  image: string;
...
const getComings = async (): Promise<{ comings: Coming[] }> => {
  try {
...
export default function ComingList() {
  const [comings, setComings] = useState<Coming[]>([]);
  const [showDialog, setShowDialog] = useState<boolean>(false);
  const [selectedComing, setSelectedComing] = useState<Coming | null>(null);
  ...
  const handleOpenDialog = (coming: Coming) => {
