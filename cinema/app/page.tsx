...
// Define the Movie type structure
interface Movie {
  id: string;
  _id: string;
  title: string;
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      try {
        ...
        const moviesWithId = (data.movies || []).map((movie: Movie, index: number) => ({
          ...movie,
          ...
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
            <div className="card glass p-6 rounded-lg shadow-lg w-11/12 md:w-1/3 text-white">
              ...
              <p className="mt-2 flex gap-1">
                <span className="font-bold">Төрөл:</span> {currentMovie.type}
              </p>
              <p className="mt-2 flex gap-1">
                <span className="font-bold">Нас:</span> {currentMovie.age}
              </p>
              <p className="mt-2 flex gap-1">
                <span className="font-bold">Үргэлжлэх хугацаа:</span> {currentMovie.duration}
              </p>
